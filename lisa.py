...
from pyspark.sql.functions import when, trim, concat
from pyspark.sql.functions import lower, when, current_date, greatest
from pyspark.sql.functions import col, row_number, first, lpad
...
########################################################################
### Renaming amnd Removing Columns
########################################################################

__all_cols = {
    'lisanr' : 'lisa_id',
    'kvkdosnr' : 'case_id',
    'kvkvestnr' : 'establishment_id',
    'naam' : 'establishment_name',
    'straatkort' : 'establishment_street_short',
    'straat' : 'establishment_street',
    'huisnr' : 'establishment_number',
    'huisletter' : 'establishment_letter',
    'toev' : 'establishment_extension',
    'postcode' : 'establishment_postcode',
    'plaats' : 'establishment_city',
    'manf' : 'employees_male_fulltime',
    'vrouwf' : 'employees_female_fulltime',
    'manp' : 'employees_male_parttime',
    'vrouwp' : 'employees_female_parttime',
    'banen' : 'number_of_jobs',
    'wpft' : 'employees_fulltime',
    'wppt' : 'employees_parttime',
    'mannen' : 'employees_male',
    'vrouwen' : 'employees_female',
    'grtklas' : 'establishment_size_category',
    'enqtype' : 'lisa_survey_type',
    'bronjaar' : 'survey_content_year',
    'datumin' : 'date_lisa_provided',
    'datumverw' : 'date_lisa_processed',
    'datummut' : 'date_founded',
    'wijzein' : 'survey_registration',
    'opgehoogd' : 'counts_raised',
    'enqform' : 'survey_form_received',
    'datumeinde' : 'date_cancelled',
    'xcoord' : 'x_coordinate_rd',
    'ycoord' : 'y_coordinate_rd',
    'xykwal' : 'coordinate_source_code',
    'xy_mis' : 'coordinate_missing',
    'xykwaloms' : 'coordinate_source_description',
    'bagnaid' : 'bag_id', # Unique combination of postcode, number, letter and extension
    'bagvoid' : 'bag_object_id', # Combination of municipality code, object type and a number
    'baggdvo' : 'bag_object_usage',
    'bagoppervl' : 'bag_object_surface',
    'aantvest' : 'number_of_establishments',
    'gemnr' : 'establishment_code_municipality',
    'gemnaam' : 'establishment_name_municipality',
    'provcode' : 'establishment_code_province',
    'provnaam' : 'establishment_name_province',
    'coropcode' : 'establishment_code_corop',
    'coropnaam' : 'establishment_name_corop',
    'arbeidcode' : 'establishment_code_labor_region',
    ...
    'telefoon' : 'establishment_phone', # SHOULD BE REMOVED
    'mobiel' : 'establishment_cellphone', # SHOULD BE REMOVED
    'telbereik' : 'establishment_reachable',
    'kvkdos' : 'has_case_id',
    'kvkvsr' : 'has_establishment_id',
    'tel' : 'has_phone'
    }
...
__reqd_cols = ['case_id', 'establishment_id']

def df_ensure_columns(x, cols: list = __reqd_cols):
    """Add missing columns if mandatory.
    
    Description: In some cases the case_id and establishment_id columns
        are not available in the source data (LISA) which causes several
        functions to not function well. Therefore we will add these 
        columns and leave them blank if they don't exist.
        
    Args:
        x (Spark Dataframe): The Spark dataframe that contains the columns to be verified.
        cols (List): List object containing the column names that are mandatory.
    
    Returns:
        Spark Dataframe: Returns the dataframe after the computations.
    """
    print(f'    {datetime.datetime.now()} >> df_ensure_columns()')
    found_cols = 0
    for c in cols:
        if c in x.columns:
            print(f'    {datetime.datetime.now()}    Column {c} is found.')
            found_cols += 1
    if found_cols != len(cols):
        for c in cols:
            x = x.withColumn(c, f.lit('').cast(StringType()))

    return x

__remove_cols = ['remove_rin_nummer', 'remove_plan_naam', 'remove_wloc_type']

def df_col_remove(x, cols: list = __remove_cols):
    """Remove predefined columns from te data prior to processing.
    
    Description: Certain column should not be processed due to GDPR
        restrictions or invalid use. By default a predefined set of 
        columns is provided. When specificly None is defined, the script
        will remove all columns that start with 'remove_'.
    Args:
        x (Spark Dataframe): The Spark dataframe that contains the columns to be removed.
        cols (List): List object containing the column names to be removed.
...
__date_cols = {'date_founded' : 'yyyy-MM-dd', 
               'date_cancelled' : 'yyyy-MM-dd',
               'date_lisa_processed' : 'yyyy-MM-dd', 
               'date_lisa_adjusted' : 'yyyy-MM-dd'}
...
__bool_cols = {'counts_raised' : 'JA NEE', 
               'survey_form_received' : 'JA NEE',
               'coordinate_missing' : '1 0',
               'has_case_id' : '1 0',
               'has_establishment_id' : '1 0',
               'has_phone' : '1 0'
               }
...
__int_cols = ['employees_male_fulltime', 'employees_male_parttime', 
              'employees_female_fulltime', 'employees_female_parttime', 
              'number_of_jobs', 'employees_fulltime', 'employees_parttime', 
              'employees_male', 'employees_female', 'establishment_number']
...
__cleanup_cols = ['lisa_id', 'case_id', 'establishment_id', 
                  'establishment_name', 'establishment_street_short', 
                  'establishment_street', 'establishment_number', 
                  'establishment_letter', 'establishment_extension', 
                  'establishment_postcode', 'establishment_city', 
                  'establishment_name_municipality', 'establishment_name_province', 
                  'establishment_name_corop', 'establishment_name_labor_region', 
...
__trim_cols = ['lisa_id', 'case_id', 'establishment_id', 
                  'establishment_name', 'establishment_street_short', 
                  'establishment_street', 'establishment_number', 
                  'establishment_letter', 'establishment_extension', 
                  'establishment_postcode', 'establishment_city', 
...
def df_col_valid_kvk(x):
    """Ensure that the provided KVK identifiers are valid.
    
    Description: Sometimes the leading zero's are removed when converting
        the KVK numbers from string to interger and back to string again. 
        Thismistake is quite often made and hence we will adjust these, 
        knowing that the KVK identifier case_id should be 8 characters 
        long and thelocation number, establishment_id should be 12.
    
    Args:
        x (Spark Dataframe): The dataframe that has the possible incorrect
            identifiers to be adjusted in
    
    Returns:
        Spark Dataframe: Resulting dataframe with proper first columns.
    """
    print(f'    {datetime.datetime.now()} >> df_col_valid_kvk()')

    for c in x.columns:
        if c == 'case_id':
            x = x.withColumn('case_id', lpad(x.case_id, 8, '0'))
        if c == 'establishment_id':
            x = x.withColumn('establishment_id', lpad(x.establishment_id, 12, '0'))
...
__compare_date_cols = ['date_founded', 'date_cancelled', 'date_lisa_provided', 'date_lisa_processed']
...
def df_exact_match(x, y, mapping:dict = __test_cols, threshold:int = 0, 
                   allow_nulls:bool = False, override_matches:bool = False,
                   force_unique:bool = False, force_active:bool = False,
    ...
    if prefer_hq:
        y = y.select(*y_cols, 'hq_indicator', *__fbb_cols)
    else:
        y = y.select(*y_cols, *__fbb_cols)
    # If we are not allowed to have NULLs, remove them...
    if not allow_nulls:
        y = df_filter_empty_columns(y, y_cols)
    # If we focus on the active rows only, remove expired rows...
    if force_active:
        y = y.filter(y.fbb_deleted.isNull())
    ...
    for c in __fbb_cols:
        if c in y.columns:
            if c not in x_cols:
                ...
                new_fbb_cols.append(f'y_{c}')
            else:
                new_fbb_cols.append(c)
    if prefer_hq:
        if 'hq_idicator' not in x_cols:
            y = y.withColumnRenamed('hq_indicator', 'y_hq_indicator')
            new_fbb_cols.append('y_hq_indicator')
        else:
            new_fbb_cols.append('hq_indicator')
    ...
    if prefer_hq:
        ws  = Window.partitionBy('merge_row_id').orderBy('y_hq_indicator', 'fbb_deleted', 'fbb_max_date', 'fbb_wave_id')
    else:
        ws  = Window.partitionBy('merge_row_id').orderBy('fbb_deleted', 'fbb_max_date', 'fbb_wave_id')
    df = df.withColumn('row_iterator', f.row_number().over(ws))
    df = df.withColumn('max_row', f.max('row_iterator').over(ws))
    if force_unique:
        df = df.filter(col('max_row') == 1)
    else:
        df = df.filter(col('row_iterator') == 1)
    df = df.drop('row_iterator', 'max_row')
...
__full_duplicate_cols = ['lisa_id', 'case_id', 'establishment_id', 'establishment_name', 
                         ...
                         'establishment_postcode', 'employees_male_fulltime', 'employees_female_fulltime', 
                         'employees_male_parttime', 'employees_female_parttime', 'number_of_jobs', 
                         'employees_fulltime', 'employees_parttime', 'employees_male', 
                         'employees_female', 'survey_content_year', 'survey_registration', 'counts_raised',
                         'bag_id', 'bag_object_id', 'number_of_establishments', 'code_sbi_1']
